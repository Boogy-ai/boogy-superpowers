...
2. **Authenticate the user — now, before building.** Run the `login` tool (the plugin wires it) or `boogy login`, and walk the human through it: it prints a URL + one-time code; they open the URL, confirm the code matches, and sign in. A first-time user then picks a **handle** (lowercase `[a-z0-9-]`, 3–30 chars) — it becomes their subdomain, so everything they deploy lives at `https://<handle>.boogy.app/<service>`. **The human picks it; you never pick it for them.** It is their account's username, not the name of the app you are building, and one handle carries every service they ever build, each at its own route (`alice.boogy.app/notes/`, `alice.boogy.app/gallery/`) — so a handle taken from today's project permanently misnames their whole account. Let them choose it in the sign-in flow, or ask explicitly and wait; the `using-boogy` skill's "Choosing a handle" has the full rule. Do this early — deploying needs their token and the browser step is the one thing only the human can do.
